...
검색엔진 최적화로, 웹 사이트를 검색 엔진에서 더 잘 노출시키기 위해 수행하는 작업을 의미한다.

### 왜 SEO를 해야하는가?

위에서 언급한대로 웹 사이트를 검색 엔진에 더 잘 노출시키기 위해서다. 왜 더 잘 노출시켜야할까?

사용자들은 궁금한 것이 생기면 검색을 한다. 나의 경우엔 검색할 때 대부분 검색 리스트의 최상단에 노출된 링크를 먼저 클릭해본다. 왜냐하면 심리상 내가 검색한 내용에 대한 내용이 들어가 있을 것이라고 생각해서이다.

결국 검색엔진 최상단에 노출된다는 것은 내용의 신뢰성을 높일 수 있고 그에 따른 브랜딩이 될 수 있다고 판단된다.

### 어떻게 SEO를 개선할 수 있나?

우선 구글 검색엔진이 동작하는 방식을 알아보자
...
우리가 글을 읽을때 목차를 확인하고, 내용을 읽기 전 제목을 읽을때 명확한 제목이라면 뒤에 나오는 내용을 어느정도 추측할 수 있게 된다. 마크업을 글쓰기라고 생각한다면 글쓰기와 같이 작성해야된다고 생각한다. 그러므로 각각의 영역마다 주어지는 영역 타이틀이 있어야하며 강조 또는 대표하는 문구 또는 단어가 존재해야한다.
...
화면상으로는 변하는 것이 없다. 하지만 문서로 봤을때는 많이 깔끔해지고 명확해졌다.
...
_document.tsx에 고정적으로 작성되어 있어 모든 페이지가 “https://miniintern.com”으로 작성된다.

그렇다면 어떻게 유연하게 사용할 수 있을까? 우리 서비스는 Nextjs를 사용하고 있다. 페이지별 수정한다고 가정하면 추후에 페이지 만들때마다 head 태그에 매번 작성해주어야 한다.
...
페이지를 만들때마다 작성해야하는 것은 동일하지만 페이지 title과 description, 상세 페이지에 대한 내용 등 만 넘겨주면 canonical tag와 h1을 신경 쓸 필요가 없어져서 상세 페이지 타이틀에 대한 내용만 신경써주면 된다.
...
![refactoring_seo_9](./images/refactoring_seo_9.webp)
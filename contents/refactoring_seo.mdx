...
_document.tsx에 고정적으로 작성되어 있어 모든 페이지가 miniintern.com으로 작성된다.

그렇다면 어떻게 유연하게 사용할 수 있을까? 우리 서비스는 Nextjs를 사용하고 있다. 페이지별 수정한다고 가정하면 추후에 페이지 만들때마다 head 태그에 매번 작성해야하는 번거로움이 예상된다.

  ...
  const pickerPages = [
    {
      ...
      path: '/playground/react-picker/date',
    ...
    },
    {
      ...
      path: '/playground/react-picker/range',
    ...
    },
    {
      ...
      path: '/playground/react-picker/datetime',
    ...
    },
    {
      ...
      path: '/playground/react-picker/custom',
      ...
      <div className="grid gap-6 lg:grid-cols-2 lg:grid-cols-3">
        {pickerPages.map((picker) => (
          <Link
            key={picker.path}
            href={picker.path}
            className={`group relative overflow-hidden rounded-xl border border-neutral-700/50 ${picker.bgColor} break-all backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:border-neutral-600 hover:shadow-2xl hover:shadow-neutral-900/50`}
          >
            {/* 글래스모피즘 효과를 위한 배경 */}

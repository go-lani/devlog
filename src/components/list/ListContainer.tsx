...
export default function ListContainer({ selectedTag, postMetas }: Props) {
  return (
    <>
      <div className="border-style hidden w-[280px] shrink-0 flex-col border-r bg-zinc-800 px-4 py-[56px] lg:flex">
      ...
      <div className="w-full bg-zinc-800 pb-8">
        <h2
          className={`${shareTechMono.className} hidden-text lg:off-hidden-text bg-zinc-800 text-2xl text-app-blue-green lg:block`}
        >
          <span className="border-style inline-block border-r bg-neutral-800 p-3">
            &lt;{selectedTag} /&gt;
          </span>
        </h2>
        <ul className="lg:border-style flex flex-col bg-neutral-800 text-app-white lg:border-t">
          {postMetas.length > 0 ? (
            postMetas.map((meta) => (
              <li
